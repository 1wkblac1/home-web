    ...
    <el-form :model="form" :rules="rules" status-icon ref="form" label-width="100px" @submit.native.prevent>
      <template v-for="(item, index) in params">
        ...
        <template v-if="item.type === 'input:number'">
          <el-form-item :label="item.label" :prop="item.prop" :key="index">
            <el-input size="medium" v-model="form[item.prop]" type="number" :placeholder="item.placeholder"></el-input>
          </el-form-item>
        </template>
...
export default {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          this.$message.error('请按要求填写正确的信息')
        }
      })
    },
  },
  watch: {
    ...
    resetFlag() {
      if (this.resetFlag) {
        this.resetForm('form')
        this.$emit('update:resetFlag', false)
      }
    },
  },
}
